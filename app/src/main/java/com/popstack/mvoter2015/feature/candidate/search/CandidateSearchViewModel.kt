...
class CandidateSearchViewModel @Inject constructor(
  ...
  private var currentSearchResult: Flow<PagingData<CandidateListViewItem.SmallCandidateViewItem>>? = null

  fun search(query: String?): Flow<PagingData<CandidateListViewItem.SmallCandidateViewItem>> {
    val lastResult = currentSearchResult
    if (query == currentQueryValue && lastResult != null) {
      return lastResult
    }
    currentQueryValue = query
    val newResult: Flow<PagingData<CandidateListViewItem.SmallCandidateViewItem>> = candidatePagerFactory.createSearchPager(
      PAGE_SIZE,
      query.orEmpty()
    ).flow
      .map { pagingData ->
